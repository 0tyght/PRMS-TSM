...
export default function DashboardPage({ token, navigate }) {
  ...
  useEffect(() => {
    setSelectedRouteId((current) => {
      if (routes.some((route) => route.id === current)) return current;
  ...
  return <>
    ...
    {loading ? <LoadingState /> : <>
      ...
      <section className="waste-readiness" aria-label="ความพร้อมในการวางแผนงาน">
        ...
        <button type="button" className={routes.some((route) => !route.routeGeojson) ? "is-warning" : ""} onClick={() => navigate("resources")}><span>เส้นทางยังไม่มีแนวถนน</span><strong>{formatNumber(routes.filter((route) => !route.routeGeojson).length)}</strong><small>กำหนดเส้นทางบนแผนที่</small></button>
      </section>
