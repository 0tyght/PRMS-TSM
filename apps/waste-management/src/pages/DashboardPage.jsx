...
export default function DashboardPage({ token, navigate }) {
  ...
  {selectedRoute ? (
    ...
    <div className="waste-map-note">
      ...
      <section className="waste-panel"><header className="waste-panel__head"><div><p>การปฏิบัติงานประจำวัน</p><h2>{selectedRoute ? `แผนปฏิบัติงานเก็บขยะ: ${selectedRoute.routeCode}` : "แผนปฏิบัติงานเก็บขยะ"}</h2></div><button type="button" className="waste-button waste-button--secondary" onClick={() => navigate("plans")}>จัดการแผนปฏิบัติงานเก็บขยะ</button></header>{visiblePlans.length ? <div className="waste-table-wrap"><table className="waste-table"><thead><tr><th scope="col">แผนปฏิบัติงานเก็บขยะ</th><th scope="col">เส้นทาง</th><th scope="col">รถ / พนักงานประจำรถขยะ</th><th scope="col">ความคืบหน้า</th><th scope="col">สถานะ</th></tr></thead><tbody>{visiblePlans.map((plan) => <tr key={plan.id}><td><strong>{plan.planNo}</strong><small>{plan.scheduledDate}</small></td><td>{plan.routeName}</td><td><strong>{plan.vehicleCode}</strong><small>{plan.driverName}</small></td><td><div className="waste-progress"><i><b style={{ width: `${wasteDashboardPolicy.planProgress(plan)}%` }} /></i><span>{formatNumber(plan.collectedStops)} / {formatNumber(plan.stopTotal)} จุด</span></div></td><td><StatusBadge value={plan.status} /></td></tr>)}</tbody></table></div> : <EmptyState title={selectedRoute ? "ยังไม่มีแผนปฏิบัติงานเก็บขยะสำหรับเส้นทางที่เลือก" : "ยังไม่มีแผนปฏิบัติงานเก็บขยะในวันนี้"} detail={selectedRoute ? "เลือกทุกเส้นทางเพื่อกลับไปดูแผนทั้งหมด หรือสร้างแผนใหม่ให้เส้นทางนี้" : "สร้างแผนโดยระบุเส้นทาง รถ และพนักงานประจำรถขยะ เพื่อเริ่มติดตามการเก็บขยะ"} actionLabel="จัดการแผนปฏิบัติงานเก็บขยะ" onAction={() => navigate("plans")} />}</section>
    </>}
  </>;
}
