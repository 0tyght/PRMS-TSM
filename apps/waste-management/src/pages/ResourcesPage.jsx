...
import RouteServiceUserWorkspace from "../components/RouteServiceUserWorkspace.jsx";
...
export default function ResourcesPage({ token }) {
  ...
  async function save(value) {
    ...
    try {
      ...
      const saved = modal.item?.id ? await api.patch(path, value) : await api.post(path, value);
      setModal(null);
      await load();
      if (tab === "routes" && !modal.item?.id) {
        setRouteWorkspace({ type: "service-users", route: { ...saved, stopCount: Number(saved.stopCount || 0), serviceUserCount: Number(saved.serviceUserCount || 0) } });
      }
    } catch (requestError) {
      setError(requestError.message);
    } finally {
  ...
  return <>
    ...
    {routeWorkspace?.type === "service-users" ? <Modal title="กำหนดผู้ใช้บริการและจุดเก็บขยะในเส้นทาง" onClose={() => setRouteWorkspace(null)}><RouteServiceUserWorkspace api={api} route={routeWorkspace.route} onClose={() => setRouteWorkspace(null)} onSaved={load} /></Modal> : null}
