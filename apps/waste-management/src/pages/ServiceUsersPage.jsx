...
function RouteAssignmentWorkspace({ user, routes, suggestions, loading, saving, error, onCalculate, onConfirm, onEditLocation }) {
  ...
  return <section className="waste-assignment-workspace">
    ...
    {loading && !suggestions.length ? <LoadingState label="กำลังค้นหาเส้นทางใกล้บ้าน" /> : !suggestions.length ? <EmptyState title="ยังไม่มีเส้นทางที่พร้อมแนะนำ" detail="ต้องคำนวณแนวถนนของเส้นทางเก็บขยะก่อน จึงจะเปรียบเทียบระยะห่างจากบ้านได้" /> : <div className="waste-assignment-grid">
      ...
      <div className="waste-assignment-map"><WasteMap previousRouteGeojson={proposal && comparison.hasBaseline ? selectedRoute?.routeGeojson : null} routeGeojson={proposal?.routeGeojson || selectedRoute?.routeGeojson || null} routeStops={proposal?.stops || [{ id: user.id, stopName: `สถานที่รับบริการ ${user.fullName}`, latitude: user.latitude, longitude: user.longitude, markerRole: "HOME" }]} /><div><i />สถานที่รับบริการของผู้ใช้บริการ <span>{proposal ? "เทาเส้นเดิม · เขียวเส้นใหม่" : "เส้นสีเขียวคือเส้นทางปัจจุบัน"}</span></div></div>
    </div>}
    {selectedSuggestion && !selectedSuggestion.recommended ? <div className={`waste-assignment-warning${isSelectedRouteEligible ? "" : " is-danger"}`}><strong>{selectedSuggestion.requiresInitialSetup ? "เส้นทางนี้ยังไม่มีจุดเก็บขยะ" : isSelectedRouteEligible ? "เส้นทางนี้ไม่ใช่ตัวเลือกใกล้ที่สุด" : "สถานที่รับบริการอยู่ไกลจากเส้นทางเกินกำหนด"}</strong><span>{selectedSuggestion.requiresInitialSetup ? "เพิ่มเป็นจุดแรกได้ เมื่อมีจุดที่สองระบบจะคำนวณแนวถนนให้ตรวจและยืนยัน" : <>ห่างจากแนวเส้นทางประมาณ {Number(selectedSuggestion.distanceMeters).toLocaleString("th-TH")} เมตร {isSelectedRouteEligible ? "เจ้าหน้าที่ควรตรวจแผนที่และระยะทางใหม่ก่อนยืนยัน" : "กรุณาตรวจพิกัดบ้านหรือเลือกเส้นทางอื่น"}</>}</span></div> : null}
    {proposal ? <section className="waste-assignment-comparison"><header><div><small>ผลก่อนยืนยัน</small><h3>{proposal.stops.length === 1 ? "เส้นทางเดิมเทียบเส้นทางหลังเพิ่มจุดเก็บขยะแรก" : "เส้นทางเดิมเทียบเส้นทางหลังเพิ่มจุดเก็บขยะ"}</h3></div><span>{proposal.stops.length.toLocaleString("th-TH")} จุดเก็บ</span></header><div><article><span>ระยะทางเดิม</span><strong>{comparison.hasBaseline ? formatRouteDistance(comparison.currentDistanceMeters) : "ยังไม่มีค่าฐาน"}</strong></article><article><span>ระยะทางใหม่</span><strong>{formatRouteDistance(proposal.distanceMeters)}</strong></article><article><span>เวลาใหม่โดยประมาณ</span><strong>{formatRouteDuration(proposal.durationSeconds)}</strong></article></div><p>ระบบรักษาแนววิ่งและลำดับจุดเก็บเดิมทั้งหมด แล้วแทรกทางอ้อมจากช่วงถนนที่ใกล้ที่สุดไปยังบ้านนี้และกลับเข้าเส้นทางเดิม ข้อมูลจริงจะเปลี่ยนเมื่อกดยืนยันเท่านั้น</p></section> : null}
...
function ServiceUserForm({ initial, villages, onCancel, onSubmit, onUnlinkLine, saving }) {
  ...
  return <form className="waste-form" onSubmit={submit}>
    ...
    <footer><button type="button" className="waste-button waste-button--secondary" onClick={onCancel}>ยกเลิก</button><button className="waste-button waste-button--primary" disabled={saving}>{saving ? "กำลังบันทึก" : initial ? "บันทึกการแก้ไข" : "เพิ่มสถานที่รับบริการ"}</button></footer>
  </form>;
}
...
ประวัติผู้ใช้บริการ ค่าบริการ และประวัติการเก็บขยะจะยังอยู่ครบ`
  ...
  return <>
    ...
    {modal ? <Modal title={modal.id ? "แก้ไขสถานที่รับบริการเก็บขยะ" : "เพิ่มสถานที่รับบริการเก็บขยะ"} onClose={() => { setModal(null); setSuggestions([]); }}><ErrorNotice error={error} /><ServiceUserForm initial={modal.id ? modal : null} villages={villages} onCancel={() => { setModal(null); setSuggestions([]); }} onSubmit={save} onUnlinkLine={unlinkLine} saving={saving} /></Modal> : null}
