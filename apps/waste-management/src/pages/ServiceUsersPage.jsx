...
function RouteAssignmentWorkspace({ user, routes, suggestions, loading, saving, error, onCalculate, onConfirm, onEditLocation }) {
  ...
  return <section className="waste-assignment-workspace">
    ...
    {selectedSuggestion && !selectedSuggestion.recommended ? <div className={`waste-assignment-warning${isSelectedRouteEligible ? "" : " is-danger"}`}><strong>{isSelectedRouteEligible ? "เส้นทางนี้ไม่ใช่ตัวเลือกใกล้ที่สุด" : "จุดรับขยะอยู่ไกลจากเส้นทางเกินกำหนด"}</strong><span>ห่างจากแนวเส้นทางประมาณ {Number(selectedSuggestion.distanceMeters).toLocaleString("th-TH")} เมตร {isSelectedRouteEligible ? "เจ้าหน้าที่ควรตรวจแผนที่และระยะทางใหม่ก่อนยืนยัน" : "กรุณาตรวจพิกัดบ้านหรือเลือกเส้นทางอื่น"}</span></div> : null}
