...
function RouteAssignmentWorkspace({ user, routes, suggestions, loading, saving, error, onCalculate, onConfirm, onEditLocation }) {
  ...
  return <section className="waste-assignment-workspace">
    ...
    {proposal ? <section className="waste-assignment-comparison"><header><div><small>ผลก่อนยืนยัน</small><h3>{proposal.stops.length === 1 ? "กำหนดจุดรับขยะแรกบนแนวเส้นทางเดิม" : "เส้นทางเดิมเทียบเส้นทางหลังเพิ่มจุดรับขยะ"}</h3></div><span>{proposal.stops.length.toLocaleString("th-TH")} จุดเก็บ</span></header><div><article><span>ระยะทางเดิม</span><strong>{comparison.hasBaseline ? formatRouteDistance(comparison.currentDistanceMeters) : "ยังไม่มีค่าฐาน"}</strong></article><article><span>ระยะทางใหม่</span><strong>{formatRouteDistance(proposal.distanceMeters)}</strong></article><article><span>เวลาใหม่โดยประมาณ</span><strong>{formatRouteDuration(proposal.durationSeconds)}</strong></article></div><p>{proposal.stops.length === 1 ? "เส้นทางนี้ยังไม่มีบ้านในทะเบียน ระบบจึงคงแนวถนนทางการเดิมและเพิ่มบ้านนี้เป็นจุดเก็บแรก เมื่อมีบ้านเพิ่ม ระบบจะจัดลำดับและคำนวณแนวถนนใหม่อัตโนมัติ" : "ระบบเพิ่มจุดรับขยะนี้เข้ากับจุดเดิม เรียงลำดับใหม่ และคำนวณแนวถนนให้อัตโนมัติ"} ข้อมูลจริงจะเปลี่ยนเมื่อกดยืนยันเท่านั้น</p></section> : null}
    <footer><div>{selectedSuggestion ? <><span>{proposal ? "พร้อมยืนยัน" : "เส้นทางที่เลือก"}</span><strong>{selectedSuggestion.routeCode} · {selectedSuggestion.routeName}</strong></> : null}</div>{proposal ? <div className="waste-assignment-actions"><button type="button" className="waste-button waste-button--secondary" disabled={saving} onClick={() => setProposal(null)}>เลือกใหม่</button><button type="button" className="waste-button waste-button--primary" disabled={saving} onClick={() => onConfirm(proposal)}>{saving ? "กำลังยืนยัน…" : "ยืนยันการกำหนดเส้นทาง"}</button></div> : <button type="button" className="waste-button waste-button--primary" disabled={!selectedSuggestion || loading || saving || selectedSuggestion.id === user.routeId} onClick={async () => { const requestId = ++calculationSequence.current; const result = await onCalculate(selectedSuggestion); if (result && calculationSequence.current === requestId) setProposal(result); }}>{loading || saving ? "กำลังคำนวณเส้นทางใหม่…" : selectedSuggestion?.id === user.routeId ? "ใช้เส้นทางนี้อยู่แล้ว" : "คำนวณเส้นทางหลังเพิ่มจุดนี้"}</button>}</footer>
  </section>;
}
