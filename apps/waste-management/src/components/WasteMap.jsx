...
export default function WasteMap({ plans = [], routeGeojson = null, routeStops = [], history = [] }) {
  ...
  useEffect(() => {
    ...
    routeStops.forEach((stop, index) => {
      ...
      const marker = L.circleMarker([latitude, longitude], { radius: 8, weight: 2, color: "#fff", fillColor: stop.confirmationStatus === "COLLECTED" ? "#176323" : "#8dcc1c", fillOpacity: 1 }).addTo(layer);
    ...
    if (historyLine.length > 1) { L.polyline(historyLine, { color: "#176323", weight: 4, opacity: 0.82 }).addTo(layer); points.push(...historyLine); }
