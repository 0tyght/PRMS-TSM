...
function quickReply(items) {
  const wizardItems = (Array.isArray(items) ? items : [])
    .filter(Boolean)
    .map((action) => ({ type: "action", action }));

  return {
    // LINE accepts at most 13 quick-reply items. The complete list is kept
    // privately for the Rich Menu renderer, then removed before replying.
    items: wizardItems.slice(0, 13),
    _wizardItems: wizardItems,
  };
}
...
async function linkRichMenuForUser(lineUserId, richMenuId) {
  ...
  const response = await fetch(
    `https://api.line.me/v2/bot/user/${encodeURIComponent(
      lineUserId,
  ...
  if (!response.ok) {
    ...
    throw new Error(
      `ไม่สามารถเปลี่ยน Rich Menu ได้ (${response.status})${
        detail ? `: ${detail}` : ""
      }`,
    );
  }
...
export function buildRichSubmenuActions(envName) {
  const keyByEnvironmentName = {
    LINE_RICH_MENU_PETS_ID: "pets",
    LINE_RICH_MENU_REQUESTS_ID: "requests",
    LINE_RICH_MENU_HEALTH_ID: "health",
    LINE_RICH_MENU_OWNER_SUB_ID: "owner",
    LINE_RICH_MENU_STATUS_ID: "status",
  };

  const key = keyByEnvironmentName[envName] || String(envName || "").toLowerCase();
  const definition = buildStaticSubmenuDefinition(key);
...
async function openRichSubmenu(lineUserId, key) {
  void lineUserId;
  const definition = buildStaticSubmenuDefinition(key);
  if (!definition) throw new Error("ไม่พบเมนูย่อยที่ร้องขอ");
...
function withoutWizardHistory(draft = {}) {
  const copy = { ...draft };
  delete copy._wizardHistory;
  return copy;
}
...
async function updateSession(
  ...
  currentStep,
  draft = session.draft,
  selectedPetId = session.selectedPetId,
  options = {},
) {
  const nextDraft = withoutWizardHistory(draft);
  const history = Array.isArray(session.draft?._wizardHistory)
    ? [...session.draft._wizardHistory]
    : [];
  ...
  if (
    ...
    history.push({
      currentStep: session.currentStep,
      draft: withoutWizardHistory(session.draft),
      selectedPetId: session.selectedPetId || null,
    });
  }

  nextDraft._wizardHistory = history.slice(-30);
...
function petEligibilityClause(action) {
  if (action === "status_apply_ACTIVE") {
    return "AND p.status = 'MISSING'";
  }
  ...
  if (action === "transfer_pet") {
    return "AND p.status IN ('ACTIVE','MISSING')";
  }

  return "";
}

function petPickerTitle(action) {
  const titles = {
    pet_detail: "เลือกสัตว์เพื่อดูรายละเอียด",
    pet_update_pet: "เลือกสัตว์ที่ต้องการแก้ไข",
    vaccination_pet: "เลือกสัตว์เพื่อแจ้งวัคซีน",
    sterilization_pet: "เลือกสัตว์เพื่อแจ้งทำหมัน",
    status_menu_pet: "เลือกสัตว์เพื่อจัดการสถานะ",
...
function petPickerMessage(result, action) {
  if (!result.rows.length) {
    const statusSpecific = action.startsWith("status_apply_") || action === "transfer_pet";
    return textMessage(
      statusSpecific
        ? "ไม่พบสัตว์ที่อยู่ในสถานะซึ่งดำเนินการรายการนี้ได้"
        : "ยังไม่พบสัตว์เลี้ยงในบัญชีนี้",
      quickReply([
        ...(statusSpecific
          ? [postbackAction("กลับเมนูสถานะ", "action=status_menu", "กลับเมนูสถานะสัตว์")]
  ...
  const items = result.rows.map((pet) => {
    const species = pet.species === "CAT" ? "แมว" : "สุนัข";
    const status = PET_STATUS_LABELS[pet.status] || pet.status;
...
async function handleReferenceFlow(event, session) {
  ...
  if (session.currentStep === "PHONE" && text) {
    ...
    return { messages: [textMessage([
      ...
      `สถานะ: ${PET_STATUS_LABELS[row.status] || row.status}`,
...
async function listRequests(lineUserId, filter = "ALL", page = 0) {
  const owner = await assertOwner(lineUserId);
  const allowedFilters = new Set(["ALL", "NEED_MORE_INFO", "PENDING", "FINISHED"]);
  ...
  const requestUnionSql = `
    SELECT r.id, r.reference_no AS referenceNo, 'REGISTER_PET' AS requestType,
           r.status, r.review_note AS reviewNote, r.submitted_at AS submittedAt,
           p.name AS petName
    FROM registrations r
    INNER JOIN pets p ON p.id = r.pet_id
    WHERE r.owner_id = ?
    ...
    SELECT s.id, s.reference_no AS referenceNo, s.subject_type AS requestType,
           s.status, s.review_note AS reviewNote, s.submitted_at AS submittedAt,
           p.name AS petName
  ...
  const filterSql = {
    NEED_MORE_INFO: "WHERE requests.status = 'NEED_MORE_INFO'",
    PENDING: "WHERE requests.status IN ('SUBMITTED', 'UNDER_REVIEW')",
    FINISHED: "WHERE requests.status IN ('APPROVED', 'REJECTED', 'CANCELLED')",
    ALL: "",
  }[normalizedFilter];

  const [[countRow]] = await pool.execute(
    `SELECT COUNT(*) AS total
     FROM (${requestUnionSql}) requests
     ${filterSql}`,
    [owner.id, owner.id],
  );
  ...
  const [pageRows] = await pool.execute(
    `SELECT requests.*
     FROM (${requestUnionSql}) requests
     ${filterSql}
     ORDER BY requests.submittedAt DESC
     LIMIT ? OFFSET ?`,
    [owner.id, owner.id, pageSize, offset],
  );
  ...
  if (!pageRows.length) {
    return [
      textMessage(
        emptyLabels[normalizedFilter] || emptyLabels.ALL,
        quickReply([
          postbackAction("ติดตามเลขอ้างอิง", "action=track", "ติดตามด้วยเลขอ้างอิง"),
        ]),
      ),
    ];
  }

  const bubbles = pageRows.map((row) => ({
    type: "bubble",
    size: "micro",
    body: {
      type: "box",
      ...
      contents: [
        ...
        {
          type: "text",
          text: row.requestType === "REGISTER_PET" ? "ขึ้นทะเบียนสัตว์" : SUBJECT_LABELS[row.requestType] || row.requestType,
          size: "sm",
          wrap: true,
        },
        {
          ...
          size: "sm",
          color: row.status === "NEED_MORE_INFO" ? "#B45309" : "#087F5B",
          weight: "bold",
          wrap: true,
        },
        ...(row.reviewNote
          ? [{ type: "text", text: `เจ้าหน้าที่: ${row.reviewNote}`, size: "xs", color: "#B45309", wrap: true }]
          : []),
      ],
    },
  }));
...
async function requestDetailMessages(lineUserId, kind, id, filter = "ALL", page = 0) {
  const owner = await assertOwner(lineUserId);
  const normalizedKind = String(kind || "").toUpperCase();
  let rows;

  if (normalizedKind === "REGISTRATION") {
    [rows] = await pool.execute(
      `SELECT r.id, r.reference_no AS referenceNo, r.status,
              r.review_note AS reviewNote, r.submitted_at AS submittedAt,
              p.name AS petName, 'ขึ้นทะเบียนสัตว์' AS requestLabel
       FROM registrations r
       INNER JOIN pets p ON p.id = r.pet_id
  ...
  } else if (normalizedKind === "SUBMISSION") {
    [rows] = await pool.execute(
      `SELECT s.id, s.reference_no AS referenceNo, s.status,
              s.review_note AS reviewNote, s.submitted_at AS submittedAt,
              p.name AS petName, s.subject_type AS requestType
       FROM citizen_submissions s
       INNER JOIN pets p ON p.id = s.pet_id
  ...
  if (item.status === "NEED_MORE_INFO") {
    actions.push(postbackAction(
      "ส่งข้อมูลเพิ่มเติม",
      `action=resubmit&kind=${normalizedKind}&id=${item.id}`,
      `ส่งข้อมูลเพิ่ม ${item.referenceNo}`,
    ));
  }
  ...
  return [{
    ...
    contents: {
      ...
      header: {
        type: "box",
        layout: "vertical",
        backgroundColor: item.status === "NEED_MORE_INFO" ? "#D97706" : "#087F5B",
        paddingAll: "18px",
        contents: [
          { type: "text", text: item.referenceNo, color: "#FFFFFF", weight: "bold", size: "lg", wrap: true },
          { type: "text", text: STATUS_LABELS[item.status] || item.status, color: "#FFFFFF", size: "sm", wrap: true, margin: "sm" },
        ],
      },
      body: {
        type: "box",
        ...
        spacing: "md",
        contents: [
          { type: "text", text: `สัตว์: ${item.petName || "ไม่ระบุ"}`, size: "md", weight: "bold", wrap: true },
          { type: "text", text: `ประเภท: ${requestLabel}`, size: "sm", wrap: true },
          ...(item.submittedAt ? [{ type: "text", text: `ส่งเมื่อ: ${formatThaiDate(item.submittedAt)}`, size: "sm", color: "#64748B", wrap: true }] : []),
          ...(item.reviewNote ? [{ type: "text", text: `ข้อความจากเจ้าหน้าที่: ${item.reviewNote}`, size: "sm", color: "#B45309", wrap: true }] : []),
        ],
      },
    },
...
async function handleAction(event, state, params) {
  ...
  if (action === "pets_menu") {
    await assertOwner(lineUserId);
    return openRichSubmenu(lineUserId, "pets");
  }

  if (action === "requests_menu") {
    await assertOwner(lineUserId);
    return openRichSubmenu(lineUserId, "requests");
  }

  if (action === "health_menu") {
    await assertOwner(lineUserId);
    return openRichSubmenu(lineUserId, "health");
  }

  if (action === "owner_menu") {
    await assertOwner(lineUserId);
    return openRichSubmenu(lineUserId, "owner");
  }

  if (action === "status_menu") {
    await assertOwner(lineUserId);
    return openRichSubmenu(lineUserId, "status");
  }
  ...
  if (action.startsWith("status_pick_")) {
    ...
    return {
      messages: await showPetPicker(
        lineUserId,
        `status_apply_${nextStatus}`,
        Number(params.page || 0),
      ),
    };
  }

  if (action.startsWith("status_apply_")) {
    const nextStatus = action.slice("status_apply_".length);
  ...
  if (action === "requests" || action === "requests_all") {
    return { messages: await listRequests(lineUserId, "ALL", Number(params.page || 0)) };
  }
  if (action === "requests_need_info") {
    return {
      messages: await listRequests(lineUserId, "NEED_MORE_INFO", Number(params.page || 0)),
    };
  }
  if (action === "requests_pending") {
    return {
      messages: await listRequests(lineUserId, "PENDING", Number(params.page || 0)),
    };
  }
  if (action === "requests_finished") {
    return {
      messages: await listRequests(lineUserId, "FINISHED", Number(params.page || 0)),
    };
  }
  if (action === "requests_page") {
    return {
      messages: await listRequests(
        lineUserId,
        params.filter || "ALL",
        Number(params.page || 0),
      ),
  ...
  if (action === "request_detail") {
    return {
      messages: await requestDetailMessages(
        lineUserId,
        params.kind,
        params.id,
        params.filter || "ALL",
  ...
  if (action === "pet_manage") {
    ...
    return {
      messages: [
        textMessage(
          `จัดการ ${pet.petName} • ${PET_STATUS_LABELS[pet.status] || pet.status}`,
        ),
      ],
      wizardDefinition: {
        key: `pet-manage-${pet.id}-${pet.status}`,
        title: `จัดการ ${pet.petName}`,
        subtitle: `สถานะปัจจุบัน: ${PET_STATUS_LABELS[pet.status] || pet.status}`,
        cacheScope: "dynamic",
        choices: [
          { label: "ดูรายละเอียด", action: postbackAction("ดูรายละเอียด", `action=pet_detail&petId=${pet.id}`) },
          { label: "แก้ข้อมูลสัตว์", action: postbackAction("แก้ข้อมูลสัตว์", `action=pet_update_pet&petId=${pet.id}`) },
          { label: "แจ้งวัคซีน", action: postbackAction("แจ้งวัคซีน", `action=vaccination_pet&petId=${pet.id}`) },
          { label: "แจ้งทำหมัน", action: postbackAction("แจ้งทำหมัน", `action=sterilization_pet&petId=${pet.id}`) },
          { label: "แจ้งสถานะ", action: postbackAction("แจ้งสถานะ", `action=status_menu_pet&petId=${pet.id}`) },
        ],
      },
    };
  }
  if (action === "resubmit") {
    if (!params.kind || !params.id) {
      return { messages: await listRequests(lineUserId, "NEED_MORE_INFO") };
    }
    return {
...
export function commandToAction(command, state = null) {
  const value = normalizeNativeCommand(command);
  if (!value) return "";

  // Resolve specific destinations before the generic word "เมนู".
  if (["สุขภาพสัตว์", "ข้อมูลสุขภาพ"].some((word) => value.includes(word))) return "health_menu";
  if (["สัตว์ของฉัน", "สัตว์เลี้ยงของฉัน", "ข้อมูลสัตว์"].some((word) => value.includes(word))) return "pets_menu";
  if (["ข้อมูลเจ้าของ", "ข้อมูลของฉัน", "แก้ไขข้อมูลเจ้าของ"].some((word) => value.includes(word))) return "owner_menu";
  ...
  if (["วัคซีน", "ฉีดยา"].some((word) => value.includes(word))) return "vaccination";
  if (value.includes("ทำหมัน")) return "sterilization";
  if (["ตำแหน่ง", "พิกัด", "ที่อยู่"].some((word) => value.includes(word))) return "location";
  if (["ติดต่อ", "เบอร์โทร"].some((word) => value.includes(word))) return "contact";
  if (["วิธีใช้งาน", "บริการ"].some((word) => value.includes(word))) return "services";

  if (["เมนู", "help", "ช่วยเหลือ", "สวัสดี", "เริ่มต้น", "สถานะของฉัน"].includes(value)) {
    return "menu";
  }
...
async function dispatchSession(event, session, params) {
  ...
  await clearSession(session.lineUserId);
  return { messages: [textMessage("รายการเดิมไม่รองรับแล้ว กรุณาเริ่มใหม่", quickReply([postbackAction("เมนู", "action=menu", "เปิดเมนู")]))] };
}

async function promptCurrentSession(session) {
  const prompts = {
  ...
  if (prompt) return prompt(session);

  if (["LINK", "TRACK"].includes(session.flowType)) {
    return [
      textMessage(
        session.currentStep === "REFERENCE"
          ? "พิมพ์เลขอ้างอิง"
          : "พิมพ์เบอร์โทรศัพท์",
        cancelQuickReply(),
      ),
  ...
  if (session.flowType === "PROFILE_UPDATE") {
    if (session.draft?.field === "villageId") {
      return [
        textMessage(
          "เลือกหมู่บ้านใหม่",
          await villageQuickReply("session=profile_village"),
        ),
      ];
    }
    return [textMessage("กรุณาพิมพ์ข้อมูลใหม่", cancelQuickReply())];
  }

  return [
    textMessage(
      "ไม่พบขั้นตอนเดิม กรุณายกเลิกแล้วเริ่มใหม่",
      cancelQuickReply(),
    ),
  ];
}
...
export async function handleNativeCitizenEvent(event, suppliedState = null) {
  ...
  if (params.session === "cancel" || ["ยกเลิก", "cancel", "หยุด"].includes(textCommand)) {
    ...
    return { handled: true, messages: [textMessage("ยกเลิกรายการที่ค้างอยู่แล้ว", quickReply([postbackAction("เมนู", "action=menu", "เปิดเมนู")]))] };
  }

  if (session && params.session === "back") {
    const history = Array.isArray(session.draft?._wizardHistory)
      ? [...session.draft._wizardHistory]
      : [];
    ...
    if (!previous) {
      return {
        handled: true,
        messages: await promptCurrentSession(session),
      };
    }

    const restoredDraft = {
      ...withoutWizardHistory(previous.draft || {}),
      _wizardHistory: history,
    };
  ...
  let requestedAction = params.action || commandToAction(textCommand, state);
  ...
  if (event.type === "message" && ["image", "location"].includes(event.message?.type)) {
    return { handled: true, messages: [textMessage("ยังไม่มีรายการที่รอรับข้อมูลนี้ กรุณาเลือกบริการจากเมนูก่อน", quickReply([postbackAction("เมนู", "action=menu", "เปิดเมนู")]))] };
  }
...
export async function cleanupNativeLineState() {
  ...
  const wizardMenus = await cleanupWizardRichMenus();
