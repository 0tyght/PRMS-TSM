...
export default function LoginPage({ onLogin }) {
  ...
  async function submit(event){event.preventDefault();setBusy(true);setError("");try{if(passwordOptional&&!password){const temporaryToken="temporary-admin-access";sessionStorage.setItem("prms_access_token",temporaryToken);onLogin(temporaryToken);return}const data=await createApi(null).post("/api/auth/login",{email,password});sessionStorage.setItem("prms_access_token",data.token);onLogin(data.token)}catch(err){setError(err.message||"ไม่สามารถเข้าสู่ระบบได้")}finally{setBusy(false)}}
